...
# June 21, 2023                     #
# to convert SPDI to canonical SPDI #
#####################################
...
parser.add_argument(
    ...
    help='The name of the input file to parse (VCF, HGVS rs list, SPDI, etc.)')
parser.add_argument(
    '-t', dest='input_format', required=True,
    help='The input file format (VCF, HGVS, RS, or SPDI')
...
# new function to convert SPDI to canonical SPDI            
def batchSPDI(infile, handler=0):
    for spdi in infile:
        spdi = spdi.rstrip()
        url = api_rootURL + 'spdi/' + spdi + '/canonical_representative'
        req = apiRequest(url)
        if req and handler:
            handler(spdi, req)
        elif req:
            canon_spdi = spdi2canon(req)
            print(spdi + "\t" + canon_spdi)
